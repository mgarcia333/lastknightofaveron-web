...
export default function UnityGame() {
  ...
  // We fullscreen our own wrapper (not Unity's SetFullscreen, which promotes
  // only the <canvas>) so the exit button below stays visible as a sibling
  // inside the fullscreen element — ESC alone isn't usable here since the
  // game already binds ESC to its own pause menu.
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full aspect-video max-w-[1920px] bg-black overflow-hidden frame corner-marks"
    >
      <canvas
        id="unity-canvas"
        tabIndex={-1}
      ...
      {ready && (
        <button
          ...
          title={isFullscreen ? "Salir de pantalla completa" : "Pantalla completa (1920x1080)"}
          className="absolute top-3 right-3 z-10 flex items-center justify-center w-10 h-10 border border-[var(--gold)]/60 bg-black/60 text-[var(--gold)] hover:bg-[var(--gold)] hover:text-[var(--ink)] transition-colors"
        >
          {isFullscreen ? (
